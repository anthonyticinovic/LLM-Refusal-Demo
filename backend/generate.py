...
def stream_generate(
    ...
    Each event: {"token": str, "index": int, "projection": float,
                 "projection_post": float, "perp": list[float] | None}
    ...
    `perp` holds the positions along the demo's leftover axes, and is None
    unless `axes` was supplied. Together with `projection` it places the live
    generation in the same frame as the cached activations.
    """
    iv = None
    trace = None
    if direction is not None:
        trace = hooks_mod.ProjectionTrace(axes=axes)
    ...
    with hooks_mod.apply(lm.blocks, iv, trace):
        ...
        for step in range(max_new_tokens):
            ...
            yield {
                ...
                "perp": trace.pre_perp[step] if trace and step < len(trace.pre_perp) else None,
            }
